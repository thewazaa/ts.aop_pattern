...
class Test extends aop_pattern.Aop<testWorker> {
  ...
  public async testReturnPromise(arg1: number, arg2: string): Promise<any> {
    return this._intereceptPromise(this.testReturnPromise.name, { 0: arg1, 1: arg2 }, new Promise((ret) => {
      setTimeout(function() { ret({ arg1: arg1, arg2: arg2 }); }, 10);
    }));
  }
...
describe.only('Class', () => {
  describe('Class Method retrieves one value', () => { check(() => { return new Test().testReturnValue(1, 'a'); }, 'Test.testReturnValue', undefined) });
  describe('Class Method retrieves one value... no, exception!', () => { check(() => { return new Test().testReturnValueException(1, 'a'); }, 'Test.testReturnValueException', 'exception 1a') });

  describe('Class Method retrieves one promise', () => { check(async () => { return await new Test().testReturnPromise(1, 'a'); }, 'Test.testReturnPromise', undefined) });
  describe('Class Method retrieves one promise... no, exception!', () => { check(async () => { return await new Test().testReturnPromiseException(1, 'a'); }, 'Test.testReturnPromiseException', 'exception 1a') });

  describe('Function retrieves one value', () => { check(() => { return testMethodValue(1, 'a'); }, 'testMethodValue', undefined) });
  describe('Function retrieves one value... no, exception!', () => { check(() => { return testMethodException(1, 'a'); }, 'testMethodException', 'exception 1a') });
});

function check(call: () => {}, methodName: string, exception?: any): void {
  ...
  it('Check method exception', async () => {
    if (exception == undefined) {
      ...
      assert.equal('{"0":1,"1":"a"}', _exception[2]);
